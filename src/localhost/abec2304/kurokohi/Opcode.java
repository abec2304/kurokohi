...
import localhost.abec2304.kurokohi.util.IntegerCache;

public class Opcode {
    
    public static final String[] OPCODE_NAMES = getTable();
    public static final Hashtable opcodeMap = getMap();
    
    public static void main(String[] args) {
        for(int i = 0; i < OPCODE_NAMES.length; i++) {
            System.out.println(i + "[" + OPCODE_NAMES[i] + "]");
        }
    }
    
    public static int encode(String opcode) {
        Integer index = (Integer)opcodeMap.get(opcode);
        return index == null ? -1 : index.intValue();
    }
    
    private static String[] getTable() {
        String instructions =("nop             "
            ...
            + "jsr_w           breakpoint      null            null            null            "
            ...
            + "null            null            null            null            null            "
            + "null            null            null            null            null            "
            + "null            null            null            impdep1         impdep2         "
        );
        ...
        for(int i = 0; i < tokens.length; i++) {
            ...
            tokens[i] = "null".equals(token) ? null : token;
        }
    ...
    private static Hashtable getMap() {
        Hashtable map = new Hashtable(OPCODE_NAMES.length);
        for(int i = 0; i < OPCODE_NAMES.length; i++) {
            if(OPCODE_NAMES[i] != null)
                map.put(OPCODE_NAMES[i], IntegerCache.valueOf(i));
        }
